...
class GiveRideActivity : AppCompatActivity(), OnMapReadyCallback, GoogleMap.OnMapLongClickListener,
    ...
    private fun requestRide() {
        if (!hasInternet()) {
            ...
            getUserId()?.let {
                try {
                    show()
                    //GeoSpark create trip.
                    APIManager.createTrip(it, origin, destination, object : CreateTripCallback {
                        override fun onSuccess(trip: CreateTrip) {
